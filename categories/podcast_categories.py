PODCAST_CATEGORIES = {
    "arts": [
        ["arts", "Arts"],
        ["arts", "Books"],
        ["arts", "Design"],
        ["arts", "Fashion & Beauty"],
        ["arts", "Food"],
        ["arts", "Performing Arts"],
        ["arts", "UNKNOWN"],
        ["arts", "Visual Arts"],
    ],
    "business": [
        ["business", "Business"],
        ["business", "Careers"],
        ["business", "Entrepreneurship"],
        ["business", "Investing"],
        ["business", "Management"],
        ["business", "Marketing"],
        ["business", "Non-Profit"],
        ["business", "Tech News"],
        ["business", "UNKNOWN"],
    ],
    "comedy": [
        ["comedy", "Comedy"],
        ["comedy", "Comedy Interviews"],
        ["comedy", "Improv"],
        ["comedy", "Stand-Up"],
        ["comedy", "Stand-Up Comedy"],
        ["comedy", "UNKNOWN"],
    ],
    "education": [
        ["education", "Business"],
        ["education", "Business News"],
        ["education", "Comedy"],
        ["education", "Courses"],
        ["education", "Education"],
        ["education", "Golf"],
        ["education", "Health & Fitness"],
        ["education", "History"],
        ["education", "How To"],
        ["education", "Investing"],
        ["education", "Language Learning"],
        ["education", "News"],
        ["education", "News Commentary"],
        ["education", "Personal Journals"],
        ["education", "Politics"],
        ["education", "Running"],
        ["education", "Self-Improvement"],
        ["education", "Society & Culture"],
        ["education", "Sport"],
        ["education", "True Crime"],
        ["education", "UNKNOWN"],
    ],
    "fiction": [
        ["fiction", "Comedy Fiction"],
        ["fiction", "Drama"],
        ["fiction", "Fiction"],
        ["fiction", "Science Fiction"],
        ["fiction", "UNKNOWN"],
    ],
    "government": [["government", "Government"], ["government", "UNKNOWN"]],
    "health_and_fitness": [
        ["health_and_fitness", "Alternative Health"],
        ["health_and_fitness", "Fitness"],
        ["health_and_fitness", "Health & Fitness"],
        ["health_and_fitness", "Medicine"],
        ["health_and_fitness", "Mental Health"],
        ["health_and_fitness", "Nutrition"],
        ["health_and_fitness", "Sexuality"],
        ["health_and_fitness", "UNKNOWN"],
    ],
    "history": [["history", "History"], ["history", "UNKNOWN"]],
    "kids_and_family": [
        ["kids_and_family", "Daily News"],
        ["kids_and_family", "Education for Kids"],
        ["kids_and_family", "Kids & Family"],
        ["kids_and_family", "Parenting"],
        ["kids_and_family", "Pets & Animals"],
        ["kids_and_family", "Stories for Kids"],
        ["kids_and_family", "UNKNOWN"],
    ],
    "leisure": [
        ["leisure", "Animation & Manga"],
        ["leisure", "Automotive"],
        ["leisure", "Aviation"],
        ["leisure", "Crafts"],
        ["leisure", "Documentary"],
        ["leisure", "Games"],
        ["leisure", "Hobbies"],
        ["leisure", "Home & Garden"],
        ["leisure", "Leisure"],
        ["leisure", "Society & Culture"],
        ["leisure", "UNKNOWN"],
        ["leisure", "Video Games"],
    ],
    "music": [
        ["music", "Books"],
        ["music", "Music"],
        ["music", "Music Commentary"],
        ["music", "Music History"],
        ["music", "Music Interviews"],
        ["music", "UNKNOWN"],
    ],
    "news": [
        ["news", "Business News"],
        ["news", "Daily News"],
        ["news", "Entertainment News"],
        ["news", "News"],
        ["news", "News Commentary"],
        ["news", "Politics"],
        ["news", "Sports News"],
        ["news", "Tech News"],
        ["news", "UNKNOWN"],
    ],
    "religion_and_spirituality": [
        ["religion_and_spirituality", "Buddhism"],
        ["religion_and_spirituality", "Christianity"],
        ["religion_and_spirituality", "Hinduism"],
        ["religion_and_spirituality", "Islam"],
        ["religion_and_spirituality", "Judaism"],
        ["religion_and_spirituality", "Religion"],
        ["religion_and_spirituality", "Religion & Spirituality"],
        ["religion_and_spirituality", "Spirituality"],
        ["religion_and_spirituality", "UNKNOWN"],
    ],
    "science": [
        ["science", "Astronomy"],
        ["science", "Chemistry"],
        ["science", "Earth Sciences"],
        ["science", "Education"],
        ["science", "Life Sciences"],
        ["science", "Mathematics"],
        ["science", "Natural Sciences"],
        ["science", "Nature"],
        ["science", "Physics"],
        ["science", "Science"],
        ["science", "Social Sciences"],
        ["science", "UNKNOWN"],
    ],
    "society_and_culture": [
        ["society_and_culture", "Documentary"],
        ["society_and_culture", "Personal Journals"],
        ["society_and_culture", "Philosophy"],
        ["society_and_culture", "Places & Travel"],
        ["society_and_culture", "Relationships"],
        ["society_and_culture", "Society & Culture"],
        ["society_and_culture", "UNKNOWN"],
    ],
    "sports": [
        ["sports", "American Football"],
        ["sports", "Baseball"],
        ["sports", "Basketball"],
        ["sports", "Cricket"],
        ["sports", "Fantasy Sports"],
        ["sports", "Football"],
        ["sports", "Golf"],
        ["sports", "Hockey"],
        ["sports", "Rugby"],
        ["sports", "Running"],
        ["sports", "Soccer"],
        ["sports", "Sport"],
        ["sports", "Sports"],
        ["sports", "Swimming"],
        ["sports", "Tennis"],
        ["sports", "UNKNOWN"],
        ["sports", "Wilderness"],
        ["sports", "Wrestling"],
    ],
    "technology": [
        ["technology", "Courses"],
        ["technology", "Technology"],
        ["technology", "UNKNOWN"],
    ],
    "true_crime": [["true_crime", "True Crime"], ["true_crime", "UNKNOWN"]],
    "tv_and_film": [
        ["tv_and_film", "After Shows"],
        ["tv_and_film", "Film History"],
        ["tv_and_film", "Film Interviews"],
        ["tv_and_film", "Film Reviews"],
        ["tv_and_film", "TV & Film"],
        ["tv_and_film", "TV Reviews"],
        ["tv_and_film", "UNKNOWN"],
    ],
}
